...
- [ ] Linear β run completes to **50k steps** with no crashes.
- [ ] EMA with `decay=0.999` is used for evaluation (confirmed in logs / config).
- [ ] **FID@10k samples, NFE=50** is logged and summarized for E7.
- [ ] Loss + FID vs step plots are saved and readable. 
